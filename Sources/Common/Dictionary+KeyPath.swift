...
private protocol ValueConvertible {
    static func make(from: Any) -> Self?
}
extension Bool: ValueConvertible {
    private static func make(from: Any) -> Bool? {
        switch from {
        case let from as Int: return (from == 0 ? false : true)
        case let from as String: return Bool(stringLiteral: from)
        default: return nil
        }
    }
}
...
public extension Collection where Iterator.Element == (key: String, value: Any), Index == DictionaryIndex<String, Any> {
    ...
    private func keyPathValue<T>(_ keyPath: String, keyPathComponents: [String], object: Any) throws -> T {
        ...
        } else if let type = T.self as? ValueConvertible.Type, let jsonValue = type.make(from: value) as? T {
            return jsonValue
